...
def init_db(_):
    ...
    top_folder = os.path.dirname({{cookiecutter.project_slug}}.__file__)
    rel_file = os.path.join('db', '{{cookiecutter.project_slug}}.sqlite')
    if dev_mode:
...
def log_package_versions():
    ...
    requires = [{"name": "{{cookiecutter.project_slug}}", "version": "0.0"}, {"name": "appdirs", "version": "1.4.3"},
